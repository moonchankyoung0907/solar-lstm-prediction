...
def build_model() -> "tensorflow.keras.Model":
    """LSTM 모델 빌드

    Input(SEQ_LEN, 8) → LSTM(128) → Dropout → LSTM(64) → Dropout
    → Dense(32, relu) → Dense(16, relu) → Dense(10, linear)
    """
...
def train_model(model, X_train: np.ndarray, y_train: np.ndarray,
    ...
    """모델 학습 + EarlyStopping + ModelCheckpoint + ReduceLROnPlateau"""
    import tensorflow as tf
...
def predict(model, X: np.ndarray) -> np.ndarray:
    """예측 실행 → shape (batch, 10)"""
    return model.predict(X, verbose=0)


def save_model(model, path: pathlib.Path | str | None = None):
...
def load_model(path: pathlib.Path | str | None = None):
    import tensorflow as tf
    path = pathlib.Path(path) if path else MODEL_PATH
